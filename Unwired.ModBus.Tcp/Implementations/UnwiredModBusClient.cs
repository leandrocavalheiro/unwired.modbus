...
{    
    ...
    public (bool success, string errorCode, string error) DisconnectDevice()
    {
        try
        {
        ...
        {
            _connected = false;
            return UnwiredModBusException(exception);
        }
    }
    ...
    public (bool sucess, bool[] results, string errorCode, string error) ReadCoils(int startingAddress, int totalAddresses = 1)
    {
        ...
        catch (Exception exception)
        {
            ...
            return UnwiredModBusValueException<bool>(exception);
        }
    }
    ...
    public (bool sucess, TResult[] results, string errorCode, string error) ReadHoldings<TResult>(int startingAddress, int totalAddresses = 1, ValueTypeEnum valueType = ValueTypeEnum.Byte, RoundTypeEnum roundType = RoundTypeEnum.None, int precision = 2) where TResult : INumber<TResult>
    {
        ...
        {
            return UnwiredModBusValueException<TResult>(exception);
        }
    }
    ...
    private async Task<(bool sucess, byte[] results, string errorCode, string error)> ComunicateAsync(byte[] data)
    {
        ...
        try
        {
            ...
            var (successConnection, errorCodeConnection, errorConnection) = ManageConnection();
            if (successConnection == false)
                return (successConnection, null, errorCodeConnection, errorConnection);
    ...
    private Socket InitializeSocket()
    {
        try
        {
            return new(IPAddress.Parse(_ip).AddressFamily, SocketType.Stream, ProtocolType.Tcp);
        }
        catch (Exception)
        {
            throw;
    ...
    private (bool success, string errorCode, string error) UnwiredModBusException(Exception exception)
        => (false, "Exception", $"Device {_deviceAlias}: {exception.Message}");
    private (bool success, TResult[] results, string errorCode, string error) UnwiredModBusValueException<TResult>(Exception exception)
        => (false, null, "Exception", $"Device {_deviceAlias}: {exception.Message}");
    private (bool success, string errorCode, string error) ManageConnection(bool reconnectIfDisconnected = true)
    {
        try
        {
            if (_connected == false && reconnectIfDisconnected)
                return ConnectDevice(_ip, _port, _unitIdentifier, _timeout, _swapType, _deviceAlias);
            ...
            if (_connected == false)
            {
                ...
                error = $"Device {_deviceAlias} is disconnected";
            }

            return (_connected, errorCode, error);
        }
        catch (Exception)
        {
            throw;
